...
class KubernetesOrchestrator:
    ...
    def create_workers(self, cfg, remote_orchestrator=None):
        worker_id = 0
        ...
        """
        # Workers REMOTOS
        if remote_orchestrator and cfg.num_remote_workers > 0:
            for i in range(cfg.num_remote_workers):
                manifest = get_worker_job_template(
                    worker_id=worker_id,
                    master_host=cfg.master_ip,
                    master_port=30051,
                    pvc_name=cfg.remote_pvc_name,
                    worker_type="remote"
                )
                remote_orchestrator.apply(manifest)
                worker_id += 1
       """
