...
sync_every_override = int(os.environ.get("SYNC_EVERY_OVERRIDE", 0))
...
class TrainingLoop:
    ...
    def run(self,  worker_id, model, dataset, dataset_info, signal, stop_event):
        ...
        for epoch in range(self.cfg.epochs):
            ...
            sync_every = sync_every_override if sync_every_override > 0 else (
                self.cfg.sync_every_early if epoch == 0 else self.cfg.sync_every
            )
            print(f"Sync every: {sync_every} | Override: {sync_every_override}")
            ...
            for step, batch in enumerate(train_loader):
                ...
                if step % report_step == 0:
                    correct_predictions, total_samples = self._report_metrics(
                        worker_id, epoch, step, outputs, labels,
