    ...
    def fed_average(self, step_dir, alive_list):
        ...
        for worker_id in alive_list:
            ...
            if aggregated is None:
                original_dtypes = {k: v.dtype for k, v in worker_weights.items()}
                aggregated = {}
                for k, v in worker_weights.items():
                    # Los tensores enteros (como num_batches_tracked) no se promedian
                    if v.dtype in (torch.int32, torch.int64, torch.long):
                        aggregated[k] = v.clone()
                    else:
                        aggregated[k] = v.clone().float()
            else:
                for key in aggregated.keys():
                    if aggregated[key].dtype in (torch.int32, torch.int64, torch.long):
                        pass  # tomar el valor del primer worker, no sumar
                    else:
                        aggregated[key] += worker_weights[key].float()
            del worker_weights
            loaded += 1
        ...
        for key in aggregated.keys():
            if aggregated[key].dtype not in (torch.int32, torch.int64, torch.long):
                aggregated[key] = (aggregated[key] / loaded).to(original_dtypes[key])
