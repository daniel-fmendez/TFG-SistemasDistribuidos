...
class ModelPersistence:
    ...
    def _save_pretrained(self, model_path, global_weights):
        ...
        model_config = registry["models"].get(self.cfg.model_name)
        
        if not model_config:
            print(f"Error: El modelo {self.cfg.model_name} no existe en el registro.")
            return
        
        nlp_keywords = ["transformer", "bert", "roberta", "distilbert", "albert"]
        is_nlp = any(k in model_config["type"].lower() for k in nlp_keywords)
        
        if not is_nlp:
            torch.save(global_weights, os.path.join(model_path, "model.pt"))
            return
